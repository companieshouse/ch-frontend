...
gulp.task('update-assets-version', () => {
  ...
  return gulp.src([
    taskArguments.destination + '/ch-frontend.min.css',
    taskArguments.destination + '/ch-frontend-ie8.min.css',
    taskArguments.destination + '/ch-frontend.min.js'
  ])
    .pipe(vinylPaths(del))
    .pipe(gulpif(isDist,
      rename(obj => {
        obj.basename = obj.basename.replace(/(ch.*)(?=\.min)/g, '$1-' + pkg.version)
        return obj
      })
    ))
